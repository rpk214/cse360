classdef iRobotCreate < handle
    ...
    %
    %       Robot mode:
    %           iRC = iRobotCreate(5, 0);
    %           This will connect Matlab to the roomba through the object
    %           iRC.  The second argument is port number (/dev/ttyUSB# in
    %           linux, and COM# in windows).  An update rate must be
    %           provided for robot mode. 
    %
    %   Methods:
    %       iRobotCreate
    %       anglesensor
    ...
    methods
        function iRC = iRobotCreate(varargin)
            ...
            % Arguments should be update rate for simulation mode
            if nargin==0 || nargin==1
                sim = 1;
            % Arguments should be update rate and port for robot mode
            elseif nargin==2
                sim = 0;
            % Wrong number of arguments 
            else
                error('Constructor takes 1 or 2 arguments only!')';
            end
